...
import { estimatePriceImpact } from "../../utils/web3/estimatePriceImpact";
import {
  getImageForToken,
...
import { contractDecimals, convertFromWei } from "../../utils/web3/conversions";
import { getPositionFeeUsd } from "../../utils/positions/getPositionFeeUsd";
import { getExecutionFees } from "../../utils/web3/getExecutionFees";
import { hasPositionForSide } from "../../utils/web3/hasPositionForSide";
...
const SizeInput: React.FC<SizeInputProps> = ({
  ...
  const fetchPositionFee = async () => {
    const positionFee = getPositionFeeUsd(
      leveragedValueUsd,
      stopLossEnabled,
  ...
  const fetchExecutionFees = async () => {
    ...
    if (stopLossEnabled && takeProfitEnabled) {
      limitNumber = 2;
    } else if (stopLossEnabled || takeProfitEnabled) {
      limitNumber = 1;
    }
  ...
  useEffect(() => {
    const updateCollateralPrice = async () => {
      ...
      const price = collateralType === "ETH" ? ethPrice : usdcPrice;
  ...
  useEffect(() => {
    const fetchShouldShowTriggers = async () => {
      if (!account || !chainId || !asset?.marketId || !asset?.customId) return;
      if (activeType !== "Market") {
        ...
        const [hasPositionLong, hasPositionShort] = await Promise.all([
          hasPositionForSide(
            chainId,
            asset.marketId,
            asset.customId,
        ...
        setShowTriggers({
          showLong: !hasPositionLong,
          showShort: !hasPositionShort,
        });
      }
    };
    fetchShouldShowTriggers();
  }, [account, chainId, asset, activeType]);
  ...
  useEffect(() => {
    ...
    fetchBalance();
  }, [account, collateralType, chainId, isLong]);

  const getPriceImpact = useCallback(async () => {
    const impactValues = await estimatePriceImpact({
      chainId: chainId,
      marketId: asset?.marketId!,
      sizeDeltaUsd: leveragedValueUsd,
      indexPrice: markPrice,
      collateralPrice: collateralPrice,
      isLong: isLong,
    });

    const priceImpactFloat = contractDecimals(impactValues[0], 30);
    const impactedPriceFloat = contractDecimals(impactValues[1], 30);

    setPriceImpact(priceImpactFloat);
    setImpactedPrice(impactedPriceFloat);
  }, [
    chainId,
    asset?.marketId,
    leveragedValueUsd,
  ...
  const debouncedGetPriceImpact = useDebounce(getPriceImpact, 300);

  useEffect(() => {
    debouncedGetPriceImpact();
  }, [debouncedGetPriceImpact, leveragedValueUsd, collateralPrice, isLong]);
      ...
      {forDummyBox ? (
        ...
        <>
          ...
          <EntryButton
            ...
            fetchExecutionFees={fetchExecutionFees}
