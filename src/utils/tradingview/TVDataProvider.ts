import {
  ExchangeData,
  fetchCryptoCompareExchanges,
  fetchPairsData,
  filterSupportedExchanges,
  generateSymbol,
  makeApiRequest,
  parseFullSymbol,
} from "./cryptoCompare";
...
import { fetchGeckoTerminalOHLCV } from "./coingecko";
...
export class TVDataProvider implements IDatafeedChartApi {
  ...
  async resolveSymbol(
    ...
    try {
      ...
      const exchangesResponse = await fetchCryptoCompareExchanges();
      const exchangesData = exchangesResponse.Data as {
        [key: string]: ExchangeData;
      };

      // Fetch pairs data
      const pairsData = (await fetchPairsData()) as PairsData;

      const supportedExchanges = Object.entries(pairsData.Data).filter(
        ([exchangeName, exchangeData]) => {
          const supported =
            exchangeData.pairs && exchangeData.pairs[baseSymbol];
          return supported;
        }
      );
      ...
      const filteredExchanges = filterSupportedExchanges(
        exchangesData,
        supportedExchanges
      );

      // Determine the best exchange based on GradePoints, volume, and preferred quote currency
      const bestExchange = Object.entries(filteredExchanges).reduce(
        (best, [currentExchangeId, currentExchangeInfo]) => {
          ...
          const currentQuotes =
            supportedExchanges.find(
              ([name]) =>
                name.toLowerCase() ===
                currentExchangeInfo.InternalName.toLowerCase()
            )?.[1].pairs[baseSymbol] || [];
          const bestQuotes =
            supportedExchanges.find(
              ([name]) =>
                name.toLowerCase() ===
                bestExchangeInfo.InternalName.toLowerCase()
            )?.[1].pairs[baseSymbol] || [];
      ...
      const { full: fullSymbol } = await generateSymbol(
        bestExchange[1].InternalName,
        baseSymbol,
        quoteSymbol
      );
  ...
  async fetchSymbolData(
    ...
    try {
      // Fetch exchange info
      const exchangeData = await makeApiRequest(
        `data/v3/exchange/general?e=${exchange}`
      );
      ...
      // Fetch pair data
      const pairData = await makeApiRequest(
        `data/v3/pair/mapping/fsym?e=${exchange}&fsym=${baseSymbol}&tsym=${quoteSymbol}`
      );
  ...
  async getBars(
    ...
    const parsedSymbol = await parseFullSymbol(symbolInfo.ticker!);
    ...
    if (symbolInfo.exchange === "CoinGecko") {
      ...
      const query = Object.keys(urlParameters)
        ...
        .join("&");
      try {
        const data = await makeApiRequest(`data/${endpoint}?${query}&`);
        if (
